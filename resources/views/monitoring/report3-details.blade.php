...
@section('title', 'Свод-3 Детали - Toshkent Invest')
...
@section('content')
    <div class="report-container">
        ...
        <!-- Back Button -->
        <a href="{{ route('monitoring.report3') }}?{{ http_build_query($filters) }}" class="back-button no-print">
        ...
        <div class="header-section">
            ...
            <p>{{ $districtName }} | Ҳисобот даври: {{ $filters['date_from'] ?? '01.01.2023' }} -
                {{ $filters['date_to'] ?? date('d.m.Y') }} | Ҳисоб санаси: {{ $currentDate->format('d.m.Y') }}</p>
        </div>
        ...
        <div class="stats-grid no-print">
            ...
            @if($category === 'overdue' && isset($stats['total_planned_payment']))
            <div class="stat-card">
                <div class="stat-label">График бўйича (млрд сўм):</div>
                <div class="stat-value">{{ number_format($stats['total_planned_payment'], 1) }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Амалда тўланган (млрд сўм):</div>
                <div class="stat-value">{{ number_format($stats['total_actual_payment'], 1) }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Бажарилиш фоизи:</div>
            ...
            <table class="report-table">
                <thead>
                    <tr>
                        ...
                        <th>Бўлиб тўлаш<br>муддати (ой)</th>
                        <th>График бўйича<br>(млрд сўм)</th>
                        <th>Амалда тўланган<br>(млрд сўм)</th>
                        <th>Фоиз<br>(%)</th>
                        <th>Ҳолат</th>
                        <th>Эгаси</th>
                        <th>Субъект тури</th>
                ...
                <tbody>
                    @forelse($lots as $index => $lot)
                        @php
                            $totalPlanned = $lot->paymentSchedules->where('payment_date', '<=', $currentDate)->sum('planned_amount');
                            $totalActual = $lot->paymentSchedules->sum('actual_amount');
                            $percentage = $totalPlanned > 0 ? round(($totalActual / $totalPlanned) * 100, 1) : 0;

                            // Determine status
                            $status = 'active';
                            $statusText = 'Назоратда';
                            if ($totalActual >= $lot->sold_price) {
                                $status = 'paid';
                                $statusText = 'Тўланган';
                            } elseif ($lot->paymentSchedules->where('payment_date', '<', $currentDate)->where('actual_amount', '<', 'planned_amount')->count() > 0) {
                                $status = 'overdue';
                                $statusText = 'Муддат ўтган';
                            }
                        @endphp
                        <tr>
                            ...
                            <td>{{ $lot->installment_months ?? 0 }}</td>
                            <td class="text-right">{{ number_format($totalPlanned / 1000000000, 1) }}</td>
                            <td class="text-right">{{ number_format($totalActual / 1000000000, 1) }}</td>
                            <td class="text-right {{ $percentage >= 80 ? 'percentage-good' : ($percentage >= 50 ? 'percentage-warning' : 'percentage-danger') }}">
        ...
        <div class="report-note no-print">
            ...
            <p>Ушбу жадвал {{ $categoryName }} тўғрисида батафсил маълумотларни ўз ичига олади. Барча маблағлар миллиард
                сўм ҳисобида кўрсатилган. Жами: {{ $stats['count'] }} та участка.
                @if($category === 'overdue')
                Фоиз кўрсаткичи график бўйича тўлов режасининг амалда бажарилишини кўрсатади.
                @endif
            </p>
        </div>
    </div>
...
@push('scripts')
    <script>
        function exportToExcel() {
            ...
            window.location.href = '{{ route('monitoring.report3.details') }}?' + params;
        }
    </script>
@endpush
