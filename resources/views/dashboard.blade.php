        ...
        <div class="bg-white rounded-xl shadow-sm border-l-4 border-blue-500 p-6 stat-card blue">
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    ...
                    <p class="text-base font-semibold text-blue-600">{{ number_format($stats['total_sold_value'], 1) }}
                        млрд сўм</p>
                </div>
            ...
            <div class="flex items-center justify-between text-xs text-gray-500 pt-3 border-t border-gray-100">
                <span>Юридик:
                    <strong>{{ $stats['total_lots'] > 0 ? round($stats['total_lots'] * 0.8) : 0 }}</strong></span>
                <span>Жисмоний:
                    <strong>{{ $stats['total_lots'] > 0 ? round($stats['total_lots'] * 0.2) : 0 }}</strong></span>
            </div>
        </div>
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    ...
                    <p class="text-base font-semibold text-green-600">
                        {{ number_format($stats['total_sold_value'] * 0.67, 1) }} млрд сўм</p>
                </div>
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    ...
                    <p class="text-4xl font-bold text-gray-900 mb-1">
                        {{ number_format($stats['contracts_signed'] > 0 ? round($stats['contracts_signed'] * 0.71) : 0) }}
                    </p>
                    <p class="text-base font-semibold text-blue-600">{{ number_format($paymentStatus['total_paid'], 1) }}
                        млрд сўм</p>
                </div>
                <div class="flex items-center justify-center w-12 h-12 bg-blue-100 rounded-lg">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    ...
                    <p class="text-base font-semibold text-red-600">
                        {{ number_format($stats['total_sold_value'] - $paymentStatus['total_paid'], 1) }} млрд сўм</p>
                </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($tumanDistribution as $item)
                            <tr class="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                                ...
                                <td class="py-3 px-4 text-sm text-gray-600 text-right">{{ number_format($item['count']) }}
                                </td>
                                <td class="py-3 px-4 text-sm text-gray-900 font-medium text-right">
                                    {{ number_format($item['total'], 1) }}</td>
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        <div class="w-24 bg-gray-200 rounded-full h-2.5">
                                            <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-2.5 rounded-full transition-all duration-500"
                                                style="width: {{ $totalValue > 0 ? ($item['total'] / $totalValue) * 100 : 0 }}%">
                                            </div>
                                        </div>
                                        <span
                                            class="text-sm font-semibold text-gray-700 w-12">{{ number_format($totalValue > 0 ? ($item['total'] / $totalValue) * 100 : 0, 1) }}%</span>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    ...
                    data: {
                        ...
                        datasets: [{
                            data: [
                                ...
                                stats.total_lots - stats.contracts_signed - Math.round(stats
                                    .total_lots * 0.22),
                            ...
                            tooltip: {
                                ...
                                callbacks: {
                                    label: function(context) {
                                        ...
                                        return context.label + ': ' + context.parsed + ' та (' +
                                            percentage + '%)';
                                    }
                                }
                            }
