...
@else
    ...
    <div class="bg-white rounded-xl shadow-sm border-2 border-yellow-400 overflow-hidden">
        ...
        <form action="{{ route('contracts.store') }}" method="POST" class="p-6 space-y-4" id="contractCreationForm">
                ...
                <div class="space-y-3">
                    ...
                    <label class="relative flex cursor-pointer border-2 border-gray-300 rounded-lg p-4 hover:border-blue-500 transition-all payment-type-card group"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-600">Бўлиб тўлаш - График бўйича тўлов қилиш</p>
                        </div>
                    </label>
            ...
            {{-- STEP 3: Muddatli Payment Schedule (Conditional) --}}
            <div id="muddatliScheduleStep" class="hidden space-y-4">
                <div class="border-t pt-4">
                    <label class="block text-sm font-bold mb-3 text-gray-900">3. Тўлов графиги созламалари</label>
                </div>

                <div>
                    <label class="block text-xs font-medium mb-1 text-gray-700">График тури <span class="text-red-500">*</span></label>
                    <select name="schedule_frequency" id="scheduleFrequency"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="">Танланг...</option>
                        <option value="monthly">Ойлик (ҳар ой)</option>
                        <option value="quarterly">Чораклик (3 ойда бир марта)</option>
                        <option value="yearly">Йиллик (12 ойда бир марта)</option>
                    </select>
                </div>

                <div>
                    <label class="block text-xs font-medium mb-1 text-gray-700">Биринчи тўлов санаси <span class="text-red-500">*</span></label>
                    <input type="date" name="first_payment_date" id="firstPaymentDate"
                           value="{{ old('first_payment_date', now()->addMonth()->format('Y-m-d')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label class="block text-xs font-medium mb-1 text-gray-700">Тўловлар сони <span class="text-red-500">*</span></label>
                    <input type="number" name="number_of_payments" id="numberOfPayments" min="1" max="120"
                           value="{{ old('number_of_payments', 12) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                {{-- Payment Preview --}}
                <div id="paymentPreview" class="bg-blue-50 border border-blue-200 rounded-lg p-4 hidden">
                    <h4 class="font-bold text-blue-900 text-xs mb-3 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-white p-3 rounded-lg">
                            <span class="text-xs text-gray-600 block mb-1">Ҳар бир тўлов:</span>
                            <p class="font-bold text-sm text-blue-700" id="previewPaymentAmount">0 сўм</p>
                        </div>
                        <div class="bg-white p-3 rounded-lg">
                            <span class="text-xs text-gray-600 block mb-1">Жами тўловлар:</span>
                            <p class="font-bold text-sm text-blue-700" id="previewTotalPayments">0</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="additionalSettingsStep" class="hidden space-y-4">
                <div class="border-t pt-4">
                    <label class="block text-sm font-bold mb-3 text-gray-900">4. Қўшимча созламалар</label>
                </div>
