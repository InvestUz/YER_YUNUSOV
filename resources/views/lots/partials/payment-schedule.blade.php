            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-5 border border-blue-200">
                        ...
                        <p class="text-xs text-blue-600 mt-1">{{ number_format($lot->paid_amount, 0, '.', ' ') }} сўм
                        </p>
                    </div>
                    ...
                    @if ($lot->qualifiesForDiscount())
                        <div
                            class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-5 border border-yellow-200">
                            <div class="flex items-center justify-between mb-2">
                            ...
                            <p class="text-xs text-yellow-600 mt-1">{{ number_format($lot->discount, 0, '.', ' ') }} сўм
                            </p>
                        </div>
                    @endif
                    ...
                    <div
                        class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-5 border-2 border-green-300 shadow-md">
                        <div class="flex items-center justify-between mb-2">
                        ...
                        <p class="text-xs text-green-600 mt-1">
                            {{ number_format($lot->distributable_amount, 0, '.', ' ') }} сўм</p>
                    </div>
                </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    ...
                    <div class="space-y-2 text-sm text-blue-800">
                        @if ($lot->qualifiesForDiscount())
                            <p>• Тўланған сумма: <strong>{{ number_format($lot->paid_amount, 0, '.', ' ') }}
                                    сўм</strong></p>
                            <p>• Чегирма (20%): <strong>-{{ number_format($lot->discount, 0, '.', ' ') }} сўм</strong>
                            </p>
                            <p>• Шартномадан (80%): <strong>{{ number_format($lot->incoming_amount, 0, '.', ' ') }}
                                    сўм</strong></p>
                            <p class="pt-2 border-t border-blue-300">• <strong>Тақсимланадиган сумма = Шартномадан
                                    100%</strong></p>
                        ...
                        @else
                            <p>• Тўланған сумма: <strong>{{ number_format($lot->paid_amount, 0, '.', ' ') }}
                                    сўм</strong></p>
                            <p class="pt-2 border-t border-blue-300">• <strong>Тақсимланадиган сумма = Тўланған сумма
                                    100%</strong></p>
                        @endif
                    </div>
                </div>
                ...
                @if ($lot->contract->distributions->count() > 0)
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <p class="text-sm text-green-700">
                            Жами тақсимланган:
                            <strong>{{ number_format($lot->contract->distributions->sum('allocated_amount'), 0, '.', ' ') }}
                                сўм</strong>
                        </p>
                    </div>
                @else
                ...
                <table class="min-w-full text-xs border-collapse">
                    <thead>
                        <tr class="bg-gray-100 text-gray-700">
                            ...
                            <th colspan="3" class="px-4 py-3 border text-center font-semibold">Хисобот даврида
                                (сўм)</th>
                    ...
                    <tbody>
                        @forelse($lot->contract->paymentSchedules->sortBy('planned_date') as $index => $schedule)
                            <tr
                                class="hover:bg-blue-50 transition-colors {{ $schedule->actual_amount > 0 ? 'bg-green-50' : '' }}">
                                {{-- Row Number --}}
                                ...
                                {{-- Actual Amount --}}
                                <td
                                    class="px-4 py-3 border text-right font-semibold {{ $schedule->actual_amount > 0 ? 'text-green-700 bg-green-100' : 'text-gray-500' }}">
                                    {{ number_format($schedule->actual_amount, 2, '.', ' ') }}
                                </td>

                                {{-- Difference --}}
                                <td
                                    class="px-4 py-3 border text-right font-bold {{ $schedule->difference > 0 ? 'text-green-600' : ($schedule->difference < 0 ? 'text-red-600' : 'text-gray-500') }}">
                                ...
                                <td class="px-4 py-3 border text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        {{-- Edit Button (only if no distributions) --}}
                                        @if ($schedule->distributions()->count() === 0)
                                            <button
                                                onclick="openEditScheduleModal({{ $schedule->id }}, '{{ $index + 1 }}', '{{ $schedule->planned_date->format('Y-m-d') }}', '{{ optional($schedule->deadline_date)->format('Y-m-d') }}', {{ $schedule->planned_amount }}, {{ $schedule->actual_amount }})"
                                        ...
                                        {{-- Distribution Button (only if actual payment exists) --}}
                                        ...
                                        {{-- Clear Payment Button (admin only, if payment recorded and no distributions) --}}
                                        @if (auth()->user()->role === 'admin' && $schedule->actual_amount > 0 && $schedule->distributions()->count() === 0)
                                            <button
                                                onclick="openClearPaymentModal({{ $schedule->id }}, '{{ $index + 1 }}', '{{ $schedule->planned_date->format('Y-m-d') }}', {{ $schedule->actual_amount }})"
                                                class="inline-flex items-center gap-1 text-orange-600 hover:text-orange-800 hover:underline font-medium"
                                                title="Тўловни бекор қилиш">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        {{-- Delete Button (admin only, if no payment recorded and no distributions) --}}
                                        @if (auth()->user()->role === 'admin' && $schedule->actual_amount === 0 && $schedule->distributions()->count() === 0)
                                            <button
                                                onclick="openDeleteScheduleModal({{ $schedule->id }}, '{{ $index + 1 }}', '{{ $schedule->planned_date->format('Y-m-d') }}', {{ $schedule->planned_amount }})"
                                                class="inline-flex items-center gap-1 text-red-600 hover:text-red-800 hover:underline font-medium"
                                                title="Ўчириш">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
