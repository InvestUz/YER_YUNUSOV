...
@if ($lot->contract && $lot->contract->payment_type === 'muddatli')
    <div id="editScheduleModal"
        class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 animate-fadeIn">
        <div class="bg-white rounded-xl max-w-md w-full shadow-2xl animate-slideUp">
            {{-- Modal Header --}}
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-100">
                ...
                <div id="editPaymentWarning" class="hidden bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-sm text-yellow-800">
                            ...
                            <p class="mt-1">Тўланган сумма: <span id="editActualAmount" class="font-semibold">-</span>
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function openEditScheduleModal(scheduleId, paymentNumber, plannedDate, deadlineDate, plannedAmount, actualAmount) {
        ...
        if (actualAmount > 0) {
            ...
            document.getElementById('editActualAmount').textContent = new Intl.NumberFormat('uz-UZ').format(
                actualAmount) + ' сўм';
        } else {
    ...
    function openDeleteScheduleModal(scheduleId, paymentNumber, plannedDate, plannedAmount) {
        ...
        document.getElementById('deletePlannedAmount').textContent = new Intl.NumberFormat('uz-UZ').format(
            plannedAmount) + ' сўм';
...
@if (auth()->user()->role === 'admin' && $lot->contract)
    <div id="clearPaymentModal"
        class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 animate-fadeIn">
        <div class="bg-white rounded-xl max-w-md w-full shadow-2xl animate-slideUp">
            {{-- Modal Header --}}
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-orange-100">
                <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                ...
                <button onclick="closeClearPaymentModal()"
                    class="text-gray-400 hover:text-gray-600 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
            ...
            <div class="p-6">
                <div class="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
                    <div class="flex items-start gap-3 mb-3">
                        <svg class="w-6 h-6 text-orange-600 mt-0.5 flex-shrink-0" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                                clip-rule="evenodd" />
                        ...
                        <div>
                            <p class="font-semibold text-orange-900 mb-2">Диққат! Бу амал тўлов маълумотларини ўчиради:
                            </p>
                    ...
                    <div class="bg-white rounded p-3 text-sm text-gray-700">
                        ...
                        <div class="space-y-1">
                            ...
                            <p>• Тўланган сумма: <span id="clearActualAmount"
...
<script>
    ...
    function openClearPaymentModal(scheduleId, paymentNumber, plannedDate, actualAmount) {
        ...
        document.getElementById('clearActualAmount').textContent = new Intl.NumberFormat('uz-UZ').format(actualAmount) +
            ' сўм';
...
<div id="addScheduleModal"
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
        ...
        <form action="{{ $lot->contract ? route('contracts.add-schedule-item', $lot->contract) : '#' }}"
            method="POST" id="addScheduleForm"> @csrf
...
@if ($lot->contract)
    <div id="paymentModal"
        ...
        <div class="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-slideUp">
            {{-- Modal Header --}}
            <div
                class="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-gradient-to-r from-green-50 to-green-100 z-10">
                <div>
                    <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
